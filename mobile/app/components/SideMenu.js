...
const TheComponent = class extends Component {
  ...
  onAbout = () => {
	...
	render() {
      ...
      <AccountProvider>
        {({user})=>(
          <LinearGradient style={[{flex: 1}, Styles.statusContent, Styles.menuShadow]}
            ...
            <TouchableOpacity style={styles.closeButton}>
              ...
              <Flex>
                {!user ? (
                  <View>
                    <SideMenuLink onPress={this.linkPressed} text="Register Account" to="goRegister" icon="ios-unlock"/>
                    <SideMenuLink onPress={this.linkPressed} text="Login" to="goLogin" icon="ios-log-in"/>
                  </View>
                ) : (
                  <View>
                    <SideMenuLink onPress={this.linkPressed} text={user.firstName ? user.firstName + ' ' + user.lastName : user.username} to="goAccount" icon="ios-person"/>
                    <SideMenuLink onPress={this.logout} text="Logout" icon="ios-log-out"/>
                  </View>
								)}
                <SideMenuLink onPress={this.onAbout} text="About" image={require('../images/icons/home_active.png')} />
              </Flex>
            {/* <FormGroup>
